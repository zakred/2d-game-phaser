...
describe('Pathfinder', () => {
    ...
    describe('findPaths', () => {
        ...
        test('should return the paths between two points', () => {
            const expected = [{x: 0, y: 0}, {x: 1, y: 0}, {x: 2, y: 0}, {x: 2, y: 1}, {x: 2, y: 2}, {x: 2, y: 3}, {x: 2, y: 4}]

            expect(sut.findPaths(board, 0, 0, 2, 4)).toEqual(expected);
        });

        test('should return an array of paths between two points', () => {
            const expected = [{x: 0, y: 3}, {x: 0, y: 2}, {x: 1, y: 2}, {x: 2, y: 2}, {x: 2, y: 3}, {x: 2, y: 4}]
        ...
        test('should return the origin coordinates when moving to the same point', () => {
            ...
            const result = sut.findPaths(board, target.x, target.y, target.x, target.y)
        ...
        test('should return an empty array when there is no path between two points', () => {
            ...
            for (const noPath of noPaths) {
                result = sut.findPaths(boardWithoutPath, noPath.startX, noPath.startY, noPath.endX, noPath.endY)
        ...
        test('should return true when there is a path available', () => {
            ...
            const result = sut.isPathAvailable(board, target.startX, target.startY, target.endX, target.endY)
        ...
        test('should return false when there is a path available', () => {
            ...
            const result = sut.isPathAvailable(boardWithoutPath, target.startX, target.startY, target.endX, target.endY)
