...
class Game {
    ...
    #setInitialState() {
        ...
        if (
            this.player1.getId() === playerId ||
            this.player2.getId() === playerId
        ) {
        } else {
    ...
    addAction(playerId: string, type: ActionType, payload: any) {
        ...
        if (
            !this.platform1.isTileWithinRange(new Point(payload.x, payload.y))
        ) {
            return;
        }
        ...
        if (type === ActionType.MOVE) {
            ...
            const isPathAvailable = this.pathfinder.isPathAvailable(
                board,
